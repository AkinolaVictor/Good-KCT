...
async function copyUserLikes(req, res){
            ...
            for(let i=0; i<allUsers.length; i++){
                ...
                await axios.post(baseUrl("user/initializeUser"), {userID: currentUser.id, startUp: true}).then(async(response)=>{
                    if(response.data.successful){
                        ...
                        const bubbles = [...thisData.likes]
                        // console.log(bubbles);
                        const bubs = await LikeModel.findOne({userID: currentUser.id})
                        const num = i+1
                        const progress = (num/allUsers.length)*100
                        if(bubs === null){
                            const newUserBubbles = new LikeModel({userID: currentUser.id, bubbles})
                            ...
                            await LikeModel.updateOne({userID: currentUser.id}, {bubbles}).then(()=>{
    ...
    console.log("Likes fully completed");

}

module.exports = copyUserLikes